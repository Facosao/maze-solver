...
def BFS_light(nodes: list[Node], start: Node, end: int) -> list[int]:
    previous: list[int] = [0 for _ in range(len(nodes) + 1)]
    visited: list[int] = [0 for _ in range(len(nodes) + 1)]
    queue: list[int] = []
    ...
    while len(queue) > 0:
        ...
        if v.id == end:
            ...
            aux = v
            while True:
                if previous[aux.id] != 0:
                    path.append(aux.id)
                    aux = nodes[previous[aux.id]]
                else:
                    path.append(aux.id)
                    break

            path.reverse()
            print("S:", start.id, "E:", end, "L:", path)
            return path

        for node in v.adjacent:
            if visited[node.id] == 0:
                visited[node.id] = 1
                previous[node.id] = v.id
    ...
    while len(queue) > 0:
        ...
        if start is False:
            if current_node == v.id:
                ...
                if v not in nodes[current_node].adjacent:
                    path_to_node = BFS_light(nodes, nodes[current_node], v.id)
                    path_to_node.extend(queue)
                    queue = path_to_node
                    continue
                else:
                    print("M", v.id)
